...
/*   Updated: 2026/04/01 15:24:59 by clados-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3D.h"

static int	load_single_texture(void *mlx_ptr, t_texture *tex, char *path)
{
...
{
	if (!load_single_texture(lmx->lmx, &lmx->map_data->textures[0],
			lmx->map_data->no_texture))
		return (ft_putstr_fd("Error\nFailed to load NO texture.\n", 2), 0);
	if (!load_single_texture(lmx->lmx, &lmx->map_data->textures[1],
			lmx->map_data->so_texture))
		return (ft_putstr_fd("Error\nFailed to load SO texture.\n", 2), 0);
	if (!load_single_texture(lmx->lmx, &lmx->map_data->textures[2],
			lmx->map_data->we_texture))
		return (ft_putstr_fd("Error\nFailed to load WE texture.\n", 2), 0);
	if (!load_single_texture(lmx->lmx, &lmx->map_data->textures[3],
